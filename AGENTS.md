...
- `Backend/`: single ASP.NET Core project with `Controllers/` (`Auth`, `Users`, `Admin`, topics, sessions, attendance/QR), `Domain/` (pure entities, enums, rules), `Application/` (`Features/` Identity, Topics, Sessions, Attendance with commands, queries, validators plus `Abstractions/` + `Common/` pipeline), `Infrastructure/` (`Persistence/` EF Core context, configurations, repositories and `Security/` hashing + JWT, `Qr/` PNG generation), `Middleware/` (global RFC 7807 error handling incl. 410 Gone), `Migrations` under `Infrastructure/Persistence/` (incl. official-topics seed), `Program.cs`, and `Backend.csproj`. There is no `src/Api, Application, Domain, Infrastructure` split yet. File storage is a contract only (`IFileStorage` signed-URL flow + `StorageOptions` for Supabase; real client pending).
