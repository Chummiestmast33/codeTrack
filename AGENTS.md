...
- Validate inputs and permissions in the backend, even when the frontend also validates for user feedback. Protect administrator operations. Never expose passwords or secrets in responses, logs, or Git. Local secrets use UserSecrets for `dotnet` commands and `.env` for Compose (hierarchical names with `__`, e.g. `Jwt__Secret`); see `docs/entorno-pruebas.md` and the full key reference in `docs/secretos.md`.
