...
- `Backend/`: single ASP.NET Core project with `Controllers/` (`Auth`, `Users`, `Admin`), `Domain/` (pure entities, enums, rules), `Application/` (`Features/Identity/` commands, queries, validators and `Abstractions/` + `Common/` pipeline), `Infrastructure/` (`Persistence/` EF Core context, configurations, repositories and `Security/` hashing + JWT), `Middleware/` (global RFC 7807 error handling), `Migrations` under `Infrastructure/Persistence/`, `Program.cs`, and `Backend.csproj`. There is no `src/Api, Application, Domain, Infrastructure` split yet. File storage is a contract only (`IFileStorage` signed-URL flow + `StorageOptions` for Supabase; real client pending).
